...
__parse_resource(PyObject *obj, struct sanlk_resource **res_ret)
{
    ...
    for (i = 0; i < num_disks; i++) {
        PyObject *tuple, *path = NULL, *offset = NULL, *p = NULL;
        int offset_is_int = 0;
        ...
        if (PyTuple_Check(tuple)) {
            if (PyTuple_Size(tuple) != 2) {
                PyErr_Format(PyExc_ValueError, "Invalid resource tuple size: %ld",
                            PyTuple_Size(tuple));
                goto exit_fail;
            }
            ...
            if (!offset_is_int) {
                PyErr_Format(PyExc_ValueError, "Invalid resource offset");
                Py_XDECREF(p);
                goto exit_fail;
            }
        }
        //TODO: handle invalid resource tuple (see resource_api PR)

        if (p == NULL) {
            PyErr_Format(PyExc_ValueError, "Invalid resource path");
            goto exit_fail;
        }
...
util_module_init(PyObject* m)
{
    ...
    PyObject* align = Py_BuildValue("llll",
