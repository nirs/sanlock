...
__parse_resource(PyObject *obj, struct sanlk_resource **res_ret)
{
    ...
    for (i = 0; i < num_disks; i++) {
        ...
        strncpy(res->disks[i].path, PyBytes_AsString(path), SANLK_PATH_LEN - 1);
        Py_DECREF(path);
...
py_read_lockspace(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    long align = ALIGNMENT_1M;
    uint32_t io_timeout = 0;
    PyObject *ls_info = NULL, *path = NULL;
    struct sanlk_lockspace ls;
...
py_read_resource(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    long align = ALIGNMENT_1M;
    PyObject *rs_info = NULL, *path = NULL;
    ...
    if(rs) free(rs);
    Py_XDECREF(path);
    /* rs_info is either NULL or returned - no decref here */
    return rs_info;
}
...
py_rem_lockspace(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "O&kO&|kii", kwlist,
        convert_to_pybytes, &lockspace, &ls.host_id, pypath_converter, &path,
        &ls.host_id_disk.offset, &async, &unused)) {
        goto finally;
    }
...
py_get_lockspaces(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    for (i = 0; i < lss_count; i++) {
        ...
        /* PyList_Append incs refcount for added item */
        Py_DECREF(ls_entry);
    }
...
py_acquire(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    PyObject *disks, *version = Py_None, *lockspace = NULL, *resource = NULL;
    struct sanlk_resource *res = NULL;
...
py_read_resource_owners(PyObject *self __unused, PyObject *args, PyObject *keywds)
{
    ...
    long align = ALIGNMENT_1M;
    PyObject *disks, *ls_list = NULL, *lockspace = NULL, *resource = NULL;
    struct sanlk_resource *res = NULL;
    struct sanlk_host *hss = NULL;
...
{
    int rv = -1, i, j, n, num_args, sanlockfd = -1;
    PyObject *argslist, *path = NULL, *item = NULL, *item_path = NULL;
    ...
    for (i = 0, n = 0; i < num_args; i++) {
        ...
        if (!pypath_converter(item, &item_path)) {
            __set_exception(EINVAL, "Killpath argument not a unicode string or bytes");
            goto finally;
        }

        p = PyBytes_AsString(item_path);
        if (p == NULL) {
            __set_exception(EINVAL, "Killpath argument not a bytes string");
            goto finally;
        }
        ...
        /* prepare to next iter of argument path conversion */
        Py_DECREF(item_path);
        item_path = NULL;
    }
